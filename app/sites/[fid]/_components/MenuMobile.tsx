      ...
      {aberto && (
        <>
          ...
          <nav style={{ position: "absolute", right: 0, top: "100%", zIndex: 61, background: "var(--site-fundo)", border: "1px solid var(--site-borda)", borderRadius: 8, boxShadow: "0 12px 40px rgba(0,0,0,0.18)", minWidth: 210, padding: "6px 0", display: "flex", flexDirection: "column" }}>
            {itens.map((it) => {
              const st: React.CSSProperties = { padding: "11px 18px", fontSize: 13, letterSpacing: "0.1em", textTransform: "uppercase", color: cor, textDecoration: "none" };
              return /^https?:\/\//.test(it.href)
                ? <a key={it.id} href={it.href} target="_blank" rel="noopener noreferrer" onClick={() => setAberto(false)} style={st}>{it.label}</a>
                : <Link key={it.id} href={it.href === "/" ? (base || "/") : `${base}${it.href}`} onClick={() => setAberto(false)} style={st}>{it.label}</Link>;
            })}
          </nav>
        </>
      )}
