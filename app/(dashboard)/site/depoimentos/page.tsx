...
export default function DepoimentosPage() {
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "40px 24px" }}>
      ...
      <p style={{ fontSize: 13, color: "var(--color-text-secondary)", margin: "0 0 24px" }}>Exibidos na home do seu site, na ordem abaixo.</p>
      ...
      {form && (
        <div style={{ border: "1px solid var(--color-border-secondary)", borderRadius: 12, padding: 18, marginBottom: 20, background: "var(--color-background-secondary)" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <input value={form.nome} onChange={(e) => setForm({ ...form, nome: e.target.value })} placeholder="Nome do cliente *" style={inputStyle} />
            <textarea value={form.texto} onChange={(e) => setForm({ ...form, texto: e.target.value })} rows={4} placeholder="Texto do depoimento *" style={{ ...inputStyle, resize: "vertical" }} />
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              {form.foto_url && <img src={form.foto_url} alt="" style={{ width: 44, height: 44, borderRadius: "50%", objectFit: "cover" }} />}
              <button onClick={() => inputFileRef.current?.click()}
                ...
                {form.foto_url ? "Trocar foto" : "+ Foto do cliente (opcional)"}
              </button>
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
              ...
              <button onClick={salvar} disabled={salvando || !form.nome.trim() || !form.texto.trim()}
