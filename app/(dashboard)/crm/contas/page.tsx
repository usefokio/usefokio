...
export default function ContasBancariasPage() {
  ...
  const salvarTransferencia = async () => {
    ...
    const { error } = await createClient().from("crm_financial_entries").insert([
      { fotografo_id: fotografo.id, tipo: "despesa", status: "pago", pago_em: transfData, conta_bancaria_id: transfOrigem,  descricao: transfDesc.trim() || `Transferência → ${nomeDestino}`, valor: v, vencimento: transfData, data_competencia: transfData, internal_account_type: "transferencia" },
      { fotografo_id: fotografo.id, tipo: "receita", status: "pago", pago_em: transfData, conta_bancaria_id: transfDestino, descricao: transfDesc.trim() || `Transferência ← ${nomeOrigem}`,  valor: v, vencimento: transfData, data_competencia: transfData, internal_account_type: "transferencia" },
    ]);
