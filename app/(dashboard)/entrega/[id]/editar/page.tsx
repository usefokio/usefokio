...
export default function EditarEntregaPage() {
  ...
  const { loadDraft, saveDraft, clearDraft, hasDraft, dismissDraft } = useDraft<{
    titulo: string; clienteId: string; dataEvento: string;
    driveLink: string; renovacao: string; mensagem: string;
  }>(draftKey);
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data, error }) => {
        ...
        if (draft) {
          setTitulo(draft.titulo ?? g.titulo);
          setClienteId(draft.clienteId ?? g.cliente_id ?? "");
          setDataEvento(draft.dataEvento ?? g.data_evento ?? "");
          setDriveLink(draft.driveLink ?? g.drive_link ?? "");
          setRenovacao(draft.renovacao ?? (g.renewal_fee != null ? String(g.renewal_fee) : ""));
          setMensagem(draft.mensagem ?? g.mensagem ?? "");
        } else {
          setTitulo(g.titulo);
          setClienteId(g.cliente_id ?? "");
          setDataEvento(g.data_evento ?? "");
          setDriveLink(g.drive_link ?? "");
          setRenovacao(g.renewal_fee != null ? String(g.renewal_fee) : "");
  ...
  return (
    <div style={{ padding: "26px 30px", maxWidth: 640 }}>
      <div style={{ marginBottom: 24, display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
        <div>
          <button onClick={() => router.back()} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 13, color: "var(--color-text-secondary)", padding: 0, marginBottom: 10 }}>
            ← Voltar
          </button>
        ...
        <button
          ...
          style={{ padding: "8px 14px", borderRadius: 8, border: "0.5px solid rgba(239,68,68,0.4)", background: "rgba(239,68,68,0.06)", color: "#EF4444", fontSize: 12, fontWeight: 600, cursor: "pointer", marginTop: 28 }}
        >
          Excluir galeria
        </button>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <Field label="Taxa de renovação (R$)">
          <input type="number" min={0} step={0.01} value={renovacao} onChange={(e) => setRenovacao(e.target.value)} style={{ ...inputStyle, width: 200 }} />
        </Field>
