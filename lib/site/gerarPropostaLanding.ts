...
// `blocosIds` = os blocos escolhidos no editor ("o que entra no PDF"). Sem lista, entra tudo
// que faz sentido no papel.
export async function gerarEVincularPdf(landingId: string, blocosIds?: string[]): Promise<{ url: string; nome: string } | null> {
  ...
  const escolhidos = blocosIds?.length ? new Set(blocosIds) : null;
