...
-- ─── Senha e expiração ────────────────────────────────────────────────────────
-- A senha do álbum é a SENHA DO CLIENTE (clientes.senha_acesso) — validada na rota
-- server-side /api/album/acesso (service role), junto de STATUS + EXPIRAÇÃO, antes de
-- entregar as lâminas. Ou seja, a senha nunca passa pelo cliente anônimo (a rota usa
-- service role e nunca devolve a senha). A coluna album_selecoes.senha_acesso NÃO é
-- usada; as policies de SELECT anon acima seguem só como defesa/compat.
